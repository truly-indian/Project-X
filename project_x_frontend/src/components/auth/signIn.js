...
const SignIn = () => {
    ...
    const onLogIn = async () => {
        try {
            if (!email || !password) throw {error: 'empty userId or password'}
            const user = await signIn({ email, password });
            Cookies.set('userToken', user.data.token)
            Cookies.set('userInfo', JSON.stringify({email}));
            router.push('/home');
        } catch (error) {
            console.log('error while signing user!!: ', error);
            setShowToast(true);
            setToastMessage({ status: 'error', text: 'Failed to SignIn, Pelase Try Again' });
        }
    };
        ...
        <div className="flex items-center justify-center h-screen w-full px-5 sm:px-0">
            <div className="flex bg-white rounded-lg shadow-lg border overflow-hidden max-w-sm lg:max-w-4xl w-full">
                ...
                <div className="w-full p-8 lg:w-1/2">
                    ...
                    <div className="mt-8">
                        <button onClick={onLogIn} className="bg-blue-700 text-white font-bold py-2 px-4 w-full rounded hover:bg-blue-600">
                            Login
                        </button>
                    </div>
                    <a
